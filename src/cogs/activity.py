    ...
    async def on_ready(self):
        ...
        with open("src/cogs/datas/activity.json", "r") as f:
            ...
            if activity.status == 'online':
                ...
                await self.bot.change_presence(status=discord.Status.dnd, activity=discord.Game(game))
            elif activity.status == 'invisible':
                await self.bot.change_presence(status=discord.Status.invisible, activity=discord.Game(game))
            else:
