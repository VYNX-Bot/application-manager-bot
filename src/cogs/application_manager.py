...
class ApplicationManager(commands.Cog):
    def __init__(self, bot):
        print("ApplicationManager is loaded.")
        super().__init__(bot)
        self.bot = bot
        ...
        self.roles = {
            "support": [927257982931173417, 927257821379174420, 911930501613359124],
            "staff": [928281498925297724, 911930501613359124],
        }
    ...
    async def apply(self, ctx, apply_: str = ""):
        ...
        if apply_ == "":
            ...
            return await ctx.send(
                embed=discord.Embed(
                    ...
                    description=f"You cannnot supply empty application name.\nAvailable applications:\n\n{a}",
                    color=0x00E5FF,
                )
            )
        ...
        if apply_ not in list(self.applicable):
            ...
            return await ctx.send(
                embed=discord.Embed(
                    title="Application not found",
                    description=f"That application likely doesn't exist.\nAvailable applications:\n\n{a}",
                    color=0x00E5FF,
                )
            )
    ...
    async def staff(self, ctx):
        await ctx.author.send(
            embed=discord.Embed(
                title="Staff applying process.",
                description="I have started this application process.",
                color=0x00FFD2,
            )
        )
        ...
        await ctx.author.send(
            "You will have to answer some questions to apply for the staff position. Please Note: that you have **2 minutes** for each question."
        )
        responses = {}
        ...
        await ctx.author.send(
            "Thanks for taking some times for questions! Your application will be applyed shortly!"
        )
        now = time.time()
        async with aiofiles.open("src/cogs/datas/applications.json") as fp:
            ...
            embed.add_field(
                name="Applied when",
                value=str(datetime.fromtimestamp(now)).replace("-", "/"),
            )
            embed.add_field(name="Type", value="staff")
            embed.set_footer(
                text=f"You can accept this application by do `a!accept {id}`"
            )
    ...
    @commands.command()
    async def accept(
        self, ctx, app_id: str = "", *, reason: str = "No reason provided"
    ):
        """
        This command for accepting the application.
        """
        if app_id == "":
            return await ctx.send("You need to supply an application id!")
        roles = []
        ...
        for role in ctx.author.roles:
            if role.id == 911930082673696798:
                ...
                if app_id not in list(db):
                    return await ctx.send(
                        "There's no guy with that ID? What do you mean?"
                    )
    ...
    async def staff_accept(self, ctx, db, app_id, reason):
        ...
        await user.send(
            embed=discord.Embed(
                title=f"Congratulations! Your staff application is accepted! in {ctx.guild.name} by {ctx.author.mention}",
                description=f"Congratulations! Your staff application is accepted! in {ctx.guild.name} by {ctx.author.mention}\nReason:\n{reason}",
            )
        )
        del db[app_id]
        async with aiofiles.open("src/cogs/datas/applications.json", "w") as fp:
            await fp.write(json.dumps(db, indent=4))
        await ctx.send(
            "Successfully applied application and deleted from application database."
        )

    async def support(self, ctx):
        await ctx.author.send(
            embed=discord.Embed(
                title="Support applying process.",
                description="what ever sentence u want",
            )
        )
        questions = self.question["support"]
        await ctx.send("Check your direct messages!👀")
        await ctx.author.send(
            "You will have to have to answer some questions to apply for the support position. Please Note: that you have **2 minutes** for each question."
        )
        responses = {}
        for question in questions:
            await ctx.author.send(f"Question: {question}")
            ...
            if response in ["", " ", None]:
                return await ctx.author.send(
                    "You can't return an empty answer! Start over!"
                )
            responses[question] = response.content
        await ctx.author.send(
            "Thanks for taking some times for questions! Your application will be applyed shortly!"
    ...
    async def support_accept(self, ctx, db, app_id, reason):
        user = ctx.guild.get_member(int(db[app_id]["name"].strip("<@>")))
        for role in ctx.guild.roles:
        ...
        await user.send(
            embed=discord.Embed(
                title=f"Congratulations! Your support application is accepted! in {ctx.guild.name} by {ctx.author.mention}",
                description=f"Congratulations! Your support application is accepted! in {ctx.guild.name} by {ctx.author.mention}\nReason:\n{reason}",
            )
        )
        del db[app_id]
        async with aiofiles.open("src/cogs/datas/applications.json", "w") as fp:
    ...
    async def applications(self, ctx):
        ...
        a = "\n".join(list(self.applicable))
        await ctx.send(
            embed=discord.Embed(title="Available Applications", description=f"{a}")
        )
        ...
        for role in ctx.author.roles:
            if role.id == 911930082673696798:
                ...
                for id in list(db):
                    ...
                    embed.add_field(
                        name="Applied when",
                        value=str(datetime.fromtimestamp(db[id]["when"])).replace(
                            "-", "/"
                        ),
                    )
                    embed.add_field(name="Type", value="staff")
    ...
    async def decline(self, ctx, app_id: str, *, reason: str = "No reason provided"):
        ...
        await user.send(
            embed=discord.Embed(
                title=f"Your {type} application is declined!",
