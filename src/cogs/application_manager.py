...
class ApplicationManager(commands.Cog):
	...
	async def apply(self,ctx,apply_:str=""):
		...
		if apply_ not in list(self.applicable):
			a = '\n'.join(list(self.applicable))
			return await ctx.send(embed=discord.Embed(title="Application not found",description=f"That application likely doesn't exist.\nAvailable applications:\n\n{a}", color=0x00e5ff))
	...
	@commands.command()
	async def accept(self,ctx,app_id:str="",*,reason:str="No reason provided"):
		"""
