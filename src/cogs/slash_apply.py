...
class Application_Manager(commands.Cog):
    ...
    async def list_applications(self, ctx):
        ...
        for app in applications:
            embed.add_field(name=app, value=applications[app]["description"], inline=False)
    ...
    async def apply(self, ctx, app_name:str):
        ...
        try:
            if str(ctx.author.id) in list(db[str(ctx.guild.id)]["applications"][app_name]["applications"]):
                return await ctx.send(
                    embed=discord.Embed(
                        title="Error",
            ...
            for app in db[str(ctx.guild.id)]["applications"]:
                if str(ctx.author.id) in list(db[str(ctx.guild.id)]["applications"][app]["applications"]):
                    return await ctx.send(
                        embed=discord.Embed(
                            title="Error",
    ...
    async def close(self, ctx, app_name:str):
        a = await ctx.send(embed=discord.Embed(title="Question", description="Are you sure you want to close your application submission?", color=discord.Color.yellow()))
        ...
        if str(reaction.emoji) == "✅":
            async with aiofiles.open("src/cogs/db/db.json") as fp:
                db = json.loads(await fp.read())
            del db[str(ctx.guild.id)]["applications"][app_name]["applications"][str(ctx.author.id)]
            await a.edit(embed=discord.Embed(title="Success", description=f"Your application for {app_name} closed", color=discord.Color.green()))
    ...
    async def process(self, ctx, app_name, db):
        ...
        logger = finder(ctx.guild.channels, id=db[str(ctx.guild.id)]["applications"][app_name]["log_channel"])
        if logger == None:
        ...
        applier = ctx.author
        embed = discord.Embed(title=f"{applier.name} has applied for {app_name}", color=discord.Color.green(), description=f"{applier.mention} trying to apply for {app_name} application.")
        msg = await logger.send(embed=embed)
        app_info = db[str(ctx.guild.id)]["applications"][app_name]
        questions = app_info["questions"]
        answers = {}
        counter = 0
        await applier.send(f"Hello {applier.mention}, you are applying for {app_name} for {ctx.guild.name}\nRemember! You have 1 minute for each question!")
        for question in questions:
            ...
            try:
                ...
                embed.add_field(name=counter+"."+question, value=answer.content, inline=True)
        ...
        db[str(ctx.guild.id)]["applications"][app_name]["applications"][str(applier.id)] = answers
        await applier.send("Thank you for applying, please wait for the application to be processed")
        async with aiofiles.open("src/cogs/db/db.json", "w") as fp:
