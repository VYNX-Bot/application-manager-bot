...
from typing import (TYPE_CHECKING, Coroutine, Generic, TypeVar, Union,
...
class Command(Generic[CogT]):
    ...
    def _build_arguments(
        self, interaction: discord.Interaction, state: discord.state.ConnectionState
    ) -> dict[str, Any]:
        raise NotImplementedError
...
class ContextMenuCommand(Command[CogT]):
    ...
    def _build_command_payload(self):
        payload = {"name": self.name, "type": self._type}
